...
export class AgentSessionManager {
   ...
   */
  updateSessionActivity(sessionId: string, metadata?: Partial<AgentSession['metadata']>): void {
  ...
  async terminateSession(sessionId: string): Promise<void> {
    ...
    if (session.agent) {
      try {
        ...
        console.warn(`Failed to cleanup agent for session ${sessionId}: ${error}`);
      }
    }
    ...
    if (session.mcpServer) {
      try {
        ...
        console.warn(`Failed to cleanup MCP server for session ${sessionId}: ${error}`);
      }
    }
    ...
    const envSessions = this.environmentAssignments.get(session.environment.name) || [];
    const updatedSessions = envSessions.filter(id => id !== sessionId);
    if (updatedSessions.length === 0) {
      this.environmentAssignments.delete(session.environment.name);
    } else {
      this.environmentAssignments.set(session.environment.name, updatedSessions);
    }
  ...
  listSessions(filter?: SessionFilter): AgentSession[] {
    ...
    if (filter) {
      ...
      if (filter.environmentName) {
        sessions = sessions.filter(s => s.environment.name === filter.environmentName);
      }
   ...
   */
  async cleanupIdleSessions(idleThresholdMinutes: number = 30): Promise<number> {
  ...
  getMetrics(): SessionMetrics {
    ...
    for (const session of sessions) {
      ...
      byEnvironment[session.environment.name] = (byEnvironment[session.environment.name] || 0) + 1;
   ...
   */
  private generateSessionId(agentType: string, environment: Environment): string {
  ...
  private getAgentInstanceId(agent: BaseAgent): string {
    ...
    return `${agent.constructor.name}-${Object.getOwnPropertyNames(agent).length}`;
  }
  ...
  async importSessions(sessionData: any[]): Promise<number> {
    ...
    for (const data of sessionData) {
      try {
        ...
        console.log(`Importing session ${data.id} for environment ${data.environmentName}`);
        importedCount++;
      } catch (error) {
...
export function createAgentSession(
  ...
  return globalSessionManager.createSession(agentType, environment, mcpServer, agent);
}
 ...
 */
export async function cleanupInactiveSessions(idleThresholdMinutes: number = 30): Promise<number> {
