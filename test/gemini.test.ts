  ...
  describe("generateCode", () => {
    ...
    it("should execute gemini command with correct parameters", async () => {
      ...
      expect(mockSandbox.commands.run).toHaveBeenCalledWith(
        expect.stringContaining("gemini --model gemini-2.0-flash-exp --prompt"),
    ...
    it("should use default model when none specified", async () => {
      ...
      expect(mockSandbox.commands.run).toHaveBeenCalledWith(
        expect.stringContaining(
          "gemini --model gemini-2.5-pro-preview-05-06 --prompt"
        ),
        expect.objectContaining({
          onStdout: expect.any(Function),
          onStderr: expect.any(Function),
    ...
    it("should return correct command config for code mode", () => {
      ...
      expect(commandConfig).toMatchObject({
        command: expect.stringContaining(
          "gemini --model gemini-2.0-flash-exp --prompt"
        ),
        errorPrefix: "Gemini",
        labelName: "gemini",
        labelColor: "4285F4",
