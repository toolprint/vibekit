...
export class CodexAgent {
  ...
  public async createPullRequest(): Promise<{
    ...
    if (!labelResponse.ok && labelResponse.status !== 422) {
      ...
      console.error(`Failed to create label '${labelName}': ${labelResponse.status} ${errorText}`);
    }
    ...
    if (!addLabelResponse.ok) {
      ...
      console.error(`Failed to add label '${labelName}' to PR #${prData.number}: ${addLabelResponse.status} ${errorText}`);
    }
