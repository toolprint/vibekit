...
```bash
# Core operations
vibekit-proxy start       # Start proxy server
vibekit-proxy stop        # Stop proxy server  
vibekit-proxy status      # Check server status

# With options
vibekit-proxy start -p 3000    # Custom port
vibekit-proxy start -d         # Run as daemon
```
...
```bash
# Port already in use
vibekit-proxy stop -p 8080        # Stop existing proxy
vibekit-proxy start -p 3000       # Use different port

# Connection issues  
vibekit-proxy status              # Verify proxy is running
curl http://localhost:8080/health # Test health endpoint

# Enable debug logging
VIBEKIT_DEBUG=1 vibekit-proxy start
```
...
<CardGroup cols={2}>
  <Card title="Built-in Redaction" icon="shield-check" href="/cli/redaction">
    Learn more about VibeKit's data redaction capabilities
  </Card>
  <Card title="Observability" icon="chart-line" href="/cli/observability">
  ...
  <Card title="Configuration Files" icon="gear" href="/cli/configuration-files">
