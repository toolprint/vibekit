...
const config: VibeConfig = {
  ...
  environment: {
    ...
    config: {
      ...
      registryName: "dockerhub",     // Optional: specify registry (dockerhub, ghcr, etc.)
    },
  },
};
...
- **`registryName`** (optional): Which registry to use (`"dockerhub"`, `"ghcr"`, etc.) - defaults to Docker Hub
